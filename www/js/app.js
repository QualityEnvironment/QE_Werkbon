...
const app = {
    ...
    _restoreWoData() {
        try {
            ...
            for (const [id, d] of Object.entries(slim)) {
                if (!this.woData[id]) {
                    this.woData[id] = { hours: d.hours || [], materials: d.materials || [], photos: [], notes: d.notes || '', checklist: d.checklist || null };
                }
            }
        } catch (e) {}
    },
    ...
    navigate(screenId, pushHistory = true) {
        ...
        const titles = {
            ...
            screenDagoverzicht: 'Dagoverzicht',
        ...
        backBtn.classList.toggle('visible', screenId !== 'screenPlanning' && screenId !== 'screenUitgevoerd' && screenId !== 'screenDagoverzicht');
    ...
    async openWorkorder(woId) {
        ...
        document.getElementById('clientInfo').innerHTML = `
            ...
            ${planDescription ? `<div class="info-row">
                ...
                <span class="info-value">${this.escapeHtml(planDescription)}</span>
            </div>` : ''}
            <div class="info-row">
                <span class="info-icon">📞</span>
            ...
            <div class="info-row" style="background:rgba(106,44,145,0.06);border-radius:8px;padding:8px 12px;margin-top:4px">
                <span class="info-icon">💰</span>
            ...
            ${client.id ? `<button onclick="app.openChangeVatTariff()"
    ...
    editInstallation(installationId) {
        const inst = (this._loadedInstallations || []).find(i => i.id === installationId);
    ...
    onderhoudAddToMaterials() {
        ...
        // Voeg ook verplaatsingskosten toe als apart materiaal
        if (this._ohZone && window.ONDERHOUD_DATA) {
            const verplPrice = ONDERHOUD_DATA.ZONE_VERPLAATSING[this._ohZone] || 0;
            if (verplPrice > 0) {
                const data = this.woData[this.currentWO.id];
                // Alleen toevoegen als er nog geen verplaatsingskosten in de materialen staan
                const hasVerpl = data.materials.some(m =>
                    (m.name || '').toLowerCase().startsWith('verplaatsingskosten zone'));
                if (!hasVerpl) {
                    const articleData = (ONDERHOUD_DATA.VERPLAATSING_ARTICLES &&
                        ONDERHOUD_DATA.VERPLAATSING_ARTICLES[this._ohZone]) || {};
                    const articleId = articleData.id || `verpl-zone-${this._ohZone}`;
                    this.addMaterial({
                        id: articleId,
                        name: `Verplaatsingskosten Zone ${this._ohZone}`,
                        salePrice: verplPrice,
                        unitPrice: verplPrice,
                        unit: 'stuk'
                    });
                }
            }
    ...
    _showWerkbonPreviewDirect() {
        ...
        if (this.selectedUurcode && data.hours.length > 0) {
            ...
            if (totals.klant > 0) {
                ...
                const uurPrijs = this.selectedUurcode.salePrice || this.selectedUurcode.unitPrice || 0;
                ...
                rows += `<tr style="background:rgba(106,44,145,0.05)"><td>${this.escapeHtml(this.selectedUurcode.name)}${rawStr}</td><td class="text-right">${urenDecimal}u</td><td class="text-right">${this.formatPrice(uurPrijs)}</td><td class="text-right" style="font-weight:500">${this.formatPrice(urenTotal)}</td></tr>`;
            }
        }
    ...
    async executeSubmitFlow() {
        ...
        try {
            ...
            const invoicePayload = {
                ...
                // Uren meesturen voor facturatie
                hours: data.hours.filter(h => h.duration > 0).map(h => ({
                    type: h.type || 'klant',
                    duration: h.duration,
        ...
        content.innerHTML = `
            ...
            <div style="display:flex;flex-direction:column;gap:10px;margin-bottom:16px">
                <button onclick="app.startTerminalPayment('${inv.id}', ${amount}, '${inv.paymentInstruction}', '${this.escapeHtml(inv.logicId)}')"
                    style="padding:16px;border:none;border-radius:12px;background:linear-gradient(135deg,var(--qe-purple),var(--qe-dark));
                ...
                ${terminalInfo}
                <button onclick="app.startQRPayment('${inv.id}', ${amount}, '${inv.paymentInstruction}', '${this.escapeHtml(inv.logicId)}')"
    ...
    async startTerminalPayment(invoiceId, amount, ogm, invoiceLogicId) {
        ...
        try {
            ...
            // merchantRef MOET de Robaws OGM zijn (alleen cijfers, normaal 12 lang)
            let merchantRef = (ogm || '').replace(/[^0-9]/g, '');
            if (!merchantRef) {
    ...
    _loadSubmittedWOs() {
        // Lijst werd vroeger gebruikt om dagplanningen lokaal te verbergen,
        // maar leidde tot "voor altijd verborgen" na test-werkbonnen.
        // Nu bepaalt enkel Robaws (hasWerkbon) welke planningen zichtbaar zijn.
        this.submittedWOs = [];
        try { localStorage.removeItem('qe_submitted_wos'); } catch (e) {}
    },
    ...
    loadDagoverzicht() {
        ...
        // Verzamel data uit alle ingediende werkorders van vandaag
        const today = this.currentDate.toISOString().split('T')[0];
        const submittedToday = [];

        // Kijk door submittedHours (bevat arrays van uur-entries per WO)
        for (const woId of this.submittedWOs) {
            const hours = this.submittedHours[woId];
            if (hours && hours.length > 0) {
                // Haal ook materialen op uit het originele woData (als het nog bestaat)
                const savedData = this.woData[woId] || {};
                submittedToday.push({
                    woId,
                    hours: hours,
        ...
        // Ook woData nakijken voor werkorders die vandaag uren hebben maar nog niet ingediend
        let pendingCount = 0;
        let pendingHours = 0;
        for (const [woId, d] of Object.entries(this.woData)) {
            if (this.submittedWOs.includes(woId)) continue;
            const klantUren = (d.hours || []).filter(h => h.type === 'klant');
            if (klantUren.length > 0) {
                pendingCount++;
                klantUren.forEach(h => { pendingHours += (h.duration || 0) / 60; });
            }
        }

        // Bereken totalen van ingediende werkbonnen
        let totalHours = 0;
        ...
        submittedToday.forEach(sub => {
            // Uren
            if (sub.hours) {
                sub.hours.filter(h => h.type === 'klant').forEach(h => {
                    totalHours += (h.duration || 0) / 60;
                });
            }
            // Materialen
            if (sub.materials) {
                ...
                sub.materials.forEach(m => {
                    totalMaterialCost += (m.salePrice || m.unitPrice || 0) * (m.quantity || 1);
                });
            }
            // Klant
            const wo = this.workorders.find(w => w.id === sub.woId);
            if (wo) clients.push(wo.client?.name || wo.summary || 'Onbekend');
        });

        // Geschatte omzet (uren * gemiddeld uurtarief + materiaalkosten)
        const avgHourRate = 55; // Geschat uurtarief
        totalRevenue = (totalHours * avgHourRate) + totalMaterialCost;
        ...
        html += `
            <div class="card" style="margin-bottom:16px;padding:20px;background:linear-gradient(135deg, var(--qe-darkblue), var(--qe-purple));color:#fff;border-radius:16px">
                <div style="font-size:13px;opacity:0.8;text-transform:uppercase;letter-spacing:1px;margin-bottom:12px">Dagoverzicht ${this.currentDate.toLocaleDateString('nl-BE')}</div>
                <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px">
                    <div>
                        <div style="font-size:28px;font-weight:700">${submittedToday.length}</div>
                        <div style="font-size:12px;opacity:0.8">Werkbonnen ingediend</div>
                    </div>
                    <div>
                        <div style="font-size:28px;font-weight:700">${totalHours.toFixed(1)}u</div>
                        <div style="font-size:12px;opacity:0.8">Totaal uren</div>
                    </div>
                    <div>
                        <div style="font-size:28px;font-weight:700">${totalMaterials}</div>
                        <div style="font-size:12px;opacity:0.8">Materialen gebruikt</div>
                    </div>
                    <div>
                        <div style="font-size:28px;font-weight:700">${this.formatPrice(totalRevenue)}</div>
                        <div style="font-size:12px;opacity:0.8">Geschatte omzet</div>
                    </div>
                </div>
            </div>`;
        ...
        if (pendingCount > 0) {
            html += `
                <div class="card" style="margin-bottom:12px;border-left:3px solid var(--qe-orange);background:rgba(249,157,62,0.06)">
                    <div style="font-size:14px;font-weight:600;color:var(--qe-orange)">⏳ ${pendingCount} werkorder${pendingCount > 1 ? 's' : ''} nog niet ingediend</div>
                    <div style="font-size:12px;color:var(--qe-grey);margin-top:4px">${pendingHours.toFixed(1)} uur aan werk nog niet verstuurd</div>
                </div>`;
        }
        ...
        if (submittedToday.length > 0) {
            ...
            submittedToday.forEach((sub, i) => {
                const wo = this.workorders.find(w => w.id === sub.woId);
                const clientName = wo?.client?.name || wo?.summary || 'Onbekend';
                const klantUren = (sub.hours || []).filter(h => h.type === 'klant');
                const uren = klantUren.reduce((s, h) => s + (h.duration || 0) / 60, 0);
                const matCount = (sub.materials || []).length;
                html += `
                    <div class="card" style="margin-bottom:8px">
                        <div style="display:flex;justify-content:space-between;align-items:center">
                            <div>
                                <div style="font-size:14px;font-weight:500">${this.escapeHtml(clientName)}</div>
                                <div style="font-size:12px;color:var(--qe-grey);margin-top:2px">⏱ ${uren.toFixed(1)}u · 📦 ${matCount} item${matCount !== 1 ? 's' : ''}</div>
                            </div>
                            <span style="font-size:11px;padding:3px 8px;border-radius:10px;background:var(--qe-green);color:#fff">✓ Verstuurd</span>
                        </div>
                    </div>`;
            });
            html += '</div>';
            ...
            html += `
                <div class="empty-state" style="margin-top:20px">
                    ...
                    <p>Begin met je eerste werkorder om hier je dagoverzicht te zien</p>
                </div>`;
        }
    ...
    async _pollForNewPlanning() {
        ...
        try {
            ...
            const result = await RobawsAPI.getPlanning(this.currentUser.robawsEmployeeId, date);
