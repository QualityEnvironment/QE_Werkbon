...
const APIBridge = {
    ...
    async handleAuth(params, options) {
        ...
        // Stap 1 van de PIN-flow: kijk of een email bekend is en of er al een PIN is.
        if (action === 'check-email') {
            const body = await this.parseBody(options);
            const email = (body.email || '').toLowerCase().trim();
            const known = !!RobawsAPI.EMPLOYEES[email];
            if (!known) return this.jsonResponse({ known: false }, 200);
            const hasPin = await RobawsAPI.hasPin(email);
            return this.jsonResponse({ known: true, hasPin });
        }

        // Stap 2: login met PIN. Als er nog geen PIN is, mag de gebruiker er een instellen
        // (newPin meegeven). Anders moet de meegegeven pin matchen met de opgeslagen hash.
        if (action === 'login') {
            const body = await this.parseBody(options);
            const email = (body.email || '').toLowerCase().trim();
            const pin = String(body.pin || '');
            const newPin = body.newPin ? String(body.newPin) : null;

            if (!RobawsAPI.EMPLOYEES[email]) {
                return this.jsonResponse({ success: false, error: 'Onbekend emailadres' }, 401);
            }

            const hasPin = await RobawsAPI.hasPin(email);
            if (!hasPin) {
                if (!newPin) {
                    return this.jsonResponse({ success: false, needsPinSetup: true, error: 'Stel eerst een PIN in' }, 401);
                }
                const setRes = await RobawsAPI.setPin(email, newPin);
                if (!setRes.success) {
                    return this.jsonResponse({ success: false, error: setRes.error }, 400);
                }
            } else {
                const ok = await RobawsAPI.verifyPin(email, pin);
                if (!ok) {
                    return this.jsonResponse({ success: false, error: 'PIN onjuist' }, 401);
                }
            }
        ...
        if (action === 'change-pin') {
            ...
            const user = RobawsAPI.getLoggedInUser();
            if (!user) return this.jsonResponse({ error: 'Niet ingelogd' }, 401);
            const oldOk = await RobawsAPI.verifyPin(user.email, String(body.oldPin || ''));
            if (!oldOk) return this.jsonResponse({ success: false, error: 'Huidige PIN klopt niet' }, 400);
            const setRes = await RobawsAPI.setPin(user.email, String(body.newPin || ''));
            if (!setRes.success) return this.jsonResponse({ success: false, error: setRes.error }, 400);
    ...
    async handlePlanning(params) {
        ...
        const result = await RobawsAPI.getPlanning(user.robawsEmployeeId, date);
        return this.jsonResponse(result);
    },
