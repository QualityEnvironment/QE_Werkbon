 ...
 * Genereert een QR code als SVG met de EPC standaard (European Payments Council)
 *
 * Gebaseerd op: https://www.europeanpaymentscouncil.eu/document-library/guidance-documents/quick-response-code-guidelines
 */

// Minimale QR code generator (alleen wat nodig is voor EPC data)
// Gebruikt Google Charts API als fallback, of genereert via canvas
window.EPCQR = {
    /**
    ...
    /**
     * Genereer een QR code img element via Google Charts API
     * @param {Object} opts - { iban, bic, name, amount, ogm }
    ...
    generateImgTag(opts, size) {
        ...
        return `<img src="${url}" alt="Betaal QR Code" style="width:${size}px;height:${size}px;image-rendering:pixelated" onerror="this.parentElement.innerHTML='<p style=color:#999>QR code kon niet geladen worden</p>'">`;
    },

    /**
     * Genereer QR code via canvas (fallback als Google Charts niet werkt)
     * Gebruikt een minimale QR encoder
     * @param {Object} opts
     * @param {number} size
     * @returns {string} base64 data URL of lege string bij fout
     */
    generateDataUrl(opts, size) {
        size = size || 250;
        const data = this.generateEPCData(opts);
        // Gebruik QRCode library als die geladen is
        if (typeof QRCode !== 'undefined') {
            try {
                const canvas = document.createElement('canvas');
                new QRCode(canvas, { text: data, width: size, height: size });
                return canvas.toDataURL('image/png');
            } catch (e) {
                console.warn('[QR] Canvas generatie mislukt:', e);
            }
        }
        return '';
    },
     ...
     * @param {Object} invoiceResult - response van create-invoice.php
    ...
    showPaymentScreen(invoiceResult, container) {
        ...
        // Format IBAN met spaties: BE68 5390 0754 7034
        ...
        const qrImg = this.generateImgTag({
            ...
            name: companyName,
            amount: amount,
            ogm: ogm.replace(/\+/g, ''), // Stuur enkel cijfers in QR
        }, 220);
